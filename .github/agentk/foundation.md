...
**에이전트 가이드**:
- NFR은 반드시 측정 가능한 형태로 명세해야 함
- 허용치를 명확히 정의해야 함
- NFR 미충족 시 해당 설계를 기각해야 함
...
- 하나의 시나리오가 NFR이면서 QA일 수 있음을 인지해야 함
...
- 모든 산출물은 **작업 디렉토리** 내에 생성해야 함
- **deliverables.md 참조**: 작업 디렉토리 설정 방법 및 우선순위
- 작업 디렉토리 결정 순서:
  1. 대화 중 사용자가 명시적으로 지정한 디렉토리
  2. VS Code 설정 `agentk.outputDirectory` (`.vscode/settings.json`)
  3. 사용자 전역 설정의 `agentk.outputDirectory`
  4. 기본값: `docs`
- 산출물 생성 전 반드시 작업 디렉토리를 확인하고 생성해야 함
- 파일 경로는 `{작업디렉토리}/{파일명}` 형식으로 구성

**에이전트 가이드**:
- 산출물 생성 전 `.vscode/settings.json` 파일을 읽어서 `agentk.outputDirectory` 설정 확인
...
- 각 에이전트는 자신의 활동에만 집중
- **workflow.md 참조**: 각 에이전트의 역할, 활동, 산출물 정의
- 다른 활동은 다른 에이전트에게 위임
- 명확한 책임 경계 유지
...
- 관련 산출물을 철저히 분석
- 문서 간 일관성 유지
- **deliverables.md 참조**: 각 산출물의 목적, 포함 내용, 의존 관계를 파악하여 정확한 산출물 생성
  - 어떤 산출물을 생성해야 하는지
  - 어떤 내용을 포함해야 하는지
  - 어떤 문서들을 참조해야 하는지
...
- **glossary.md 참조**: 모든 용어는 용어 사전에 정의된 의미로 사용
