...
**목적**: 구조적 문제, 설계 이슈, 구조적 관심사를 완전하게 식별
...
- 목표 품질 속성에 대한 효과 분석 (장점)
- 다른 품질 속성에 대한 영향 분석 (단점/트레이드오프)
- 구현 복잡도, 비용 분석 (단점)
- 기술적 위험 분석 (단점)
...
| ID      | 제목   | 상태   | 목표 품질 속성 | 상충   | 종속   |
| ------- | ------ | ------ | -------------- | ------ | ------ |
| CA-001  | {제목} | 설계됨 | Performance    | CA-002 | -      |
| CA-001A | {제목} | 설계됨 | Performance    | -      | CA-001 |
| CA-002  | {제목} | 설계됨 | Performance    | CA-001 | -      |
| ...     | ...    | ...    | ...            | ...    | ...    |
...
CA-001: {제목}
├─> CA-001A: {보완 후보 구조}
│ └─> CA-001A1: {2차 보완 후보 구조}
└─> CA-001B: {보완 후보 구조}

CA-002: {제목} (CA-001과 상충)
...
- ❌ 구조적 문제 식별 불완전 (완전성 원칙 위반)
- ❌ 하나의 대안만 제시 (다양성 원칙 위반)
- ❌ 단점 보완 설계 누락 (재귀적 깊이 원칙 위반)
- ❌ 설계 과정 가시화 누락 (가시화 원칙 위반)
- ❌ **candidate/candidates.md 업데이트 누락**
- ❌ **후보 구조 간 관계를 candidates.md에 반영하지 않음**
...
- **glossary.md**: 아키텍처 용어
- **deliverables.md**: 후보 구조 문서 구조
- **qualities.md**: 품질 속성 요구사항 (입력)
