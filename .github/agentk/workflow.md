...
**병렬 실행**: usecase-specifier는 여러 Use Case에 대해 병렬 수행 가능
...
- **입력**: `system.md`, `business.md`, `domain/model.md`
...
- **출력**: `candidate/CA-nnn.md` (성능 관련 후보)
- **활동**:
  - 성능 요구사항 분석
  - 성능 최적화 후보 구조 설계
  - 성능 대응 전략 명세
...
- **출력**: `candidate/CA-nnn.md` (변경 용이성 관련 후보)
- **활동**:
  - 변경 시나리오 분석
  - 변경 영향도 최소화 후보 구조 설계
  - 변경 대응 전략 명세
...
### 5.2 후보 통합 (각 아키텍트)
- **입력**: 각 아키텍트가 생성한 `candidate/CA-nnn.md`
...
**목적**: 후보 아키텍처 평가 및 최종 구조 통합

**에이전트 실행 순서**:

### 6.1 candidate-evaluator (후보 구조별 반복)
- **입력**: `candidate/candidates.md`, `candidate/CA-nnn.md`, `qualities.md`
- **출력**: `decision/CA-nnn.md`
- **활동**:
  - 각 후보 평가
  - 장단점 분석
  - 품질 속성 만족도 평가
...
- **입력**: `decision/CA-nnn.md` (모든 평가 결과)
- **출력**: `decision/decisions.md`
- **활동**:
  - 채택된 후보 구조 목록
  - 기각된 후보 구조 목록

**병렬 실행**: 모든 후보 구조 평가 후 순차 실행, 여러 후보에 대해 6.1과 6.2를 순차적으로 병렬 수행 가능
...
- **입력**: 
  ...
  - `decision/decisions.md`, `decision/CA-nnn.md`
