...
- 작업 디렉토리 결정 순서:
  ...
  2. VS Code 설정 `architectureDirectory` (`.vscode/settings.json`)
  3. 사용자 전역 설정의 `architectureDirectory`
  4. 기본값: `docs`

- 모든 구현 산출물은 **소스 디렉토리** 내에 생성해야 함
- 소스 디렉토리 결정 순서:
  1. 대화 중 사용자가 명시적으로 지정한 디렉토리
  2. VS Code 설정 `sourceDirectory` (`.vscode/settings.json`)
  3. 사용자 전역 설정의 `sourceDirectory`
  4. 기본값: `src`

**에이전트 가이드**:

- 산출물 생성 전 `.vscode/settings.json` 파일을 읽어서 `architectureDirectory` 와 `sourceDirectory` 설정 확인
